...
func TestMessagingDeliveryConfig(t *testing.T) {
	c := Default()
	if c.Messaging.MaxUnreadPerRecipient != 200 || c.Messaging.RetentionDays != 30 {
	...
	c, err := Load(p)
	if err != nil || c.Messaging.MaxUnreadPerRecipient != 12 || c.Messaging.RetentionDays != 7 {
		t.Fatalf("file = %+v %v", c, err)
	}
	t.Setenv("PUNK_MESSAGING_MAX_UNREAD_PER_RECIPIENT", "3")
	t.Setenv("PUNK_MESSAGING_RETENTION_DAYS", "0")
	c, err = Load(p)
	if err != nil || c.Messaging.MaxUnreadPerRecipient != 3 || c.Messaging.RetentionDays != 0 {
		t.Fatalf("env = %+v %v", c, err)
	}
	for _, key := range []string{"PUNK_MESSAGING_MAX_UNREAD_PER_RECIPIENT", "PUNK_MESSAGING_RETENTION_DAYS"} {
