...
- Polling: {{if .Opts.Pi}}recall the ` + "`/tasks`" + ` prefix with a max_tokens that fits your context{{else}}{{tool .Opts "list_keys"}} with prefix ` + "`/tasks`" + `, then recall only the ids you need (` + "`/tasks/<id>`" + `, ` + "`/tasks/<id>/status`" + `). Do not recall the whole ` + "`/tasks`" + ` prefix: every task body comes back and the read tool budget (8000 tokens by default) cuts the list short{{end}}. Clients that support MCP resources can subscribe to ` + "`punk://memory/<namespace>/tasks`" + ` and get a notification on every change instead of polling. From a shell, ` + "`GET /v1/namespaces/<ns>/events?prefix=/tasks`" + ` is a server-sent event stream.
