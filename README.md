...
| Codex CLI | `punk connect codex` | a `$CODEX_HOME/hooks.json` merge (SessionStart `startup\|resume`, UserPromptSubmit, PostToolUse, Stop) plus the `[features] hooks = true` flag and the punk MCP entry in `$CODEX_HOME/config.toml` inside a punk-managed block; accept the one-time hook-trust prompt | native: the same nested `hookSpecificOutput.additionalContext` envelope on SessionStart and UserPromptSubmit; set `[tui] terminal_title = []` to suppress upstream spinner title re-emission (the original user confirmed this stopped visible repetition) (see `docs/investigations/codex-0.153.4-terminal-spam.md`) |
