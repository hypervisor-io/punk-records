...
20. Re-read list_tasks before taking another task. If nothing is ready, use await_tasks(timeout_seconds=45): this client cancels MCP calls at about 60 seconds, and the server's 300-second maximum does not extend that deadline. After a timeout, inspect the returned board and /answers/<id>, then retry with a shorter wait; never repeat an oversized wait. Do not busy-poll.
