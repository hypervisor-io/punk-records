...
20. Re-read list_tasks before taking another task or waiting; await_tasks watches future changes, so inspect already-ready work first. If nothing is ready, use await_tasks(timeout_seconds=45). Earlier clients canceled longer calls at about 60 seconds; see /conventions/mcp-waits for the current per-client setting and reload status. The server maximum does not extend the effective client deadline. A timed-out call returns no board: call list_tasks once, inspect ready work and /answers/<id>, then wait again only when nothing is ready. Never repeat an oversized wait or resubmit a rejected unchanged commit. Do not busy-poll.
